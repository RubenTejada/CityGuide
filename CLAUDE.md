...
company's, and they cost no LLM tokens. A branch is named by the part that tells it apart
from its siblings (`BranchNaming`): Google calls most branches by the chain ("Banreservas"
twenty-seven times), so the chain is stripped from the name ("BanReservas Torre" → "Torre")
and, when nothing is left, the street line of the address is used instead. The frontend
(`branchDisplayName`) prefixes the chain back on wherever a branch appears alone. The rating/photo backfill also covers `mall`
