...
# (Social:InstagramUserId, Social:AccessToken). The account is the place's "Instagram"
# field, else its website when that is an Instagram profile. Reads the CMS and Meta,
# writes nothing and spends nothing — no Google request, no model token. Without the
...
`InstagramFeeds.HandleOf` reads the handle out of the place's `instagram` field — a bare
handle, "@cuenta" or the profile's address — and, when that is blank, out of that same
stored website (a bare "instagram.com/…", a plain "@cuenta"). The field exists
(`EnsurePlaceInstagramSchemaAsync`, invariant like `website`) for the place whose website
is a real site and whose programme is still announced on the feed alone: the Gran Teatro
del Cibao has no site of its own, the Ministry of Culture's page stores an empty events
archive, and no ticket portal lists it (Fever carries only waiting lists), so its
Instagram is the one source and the Santiago seed table names it, written onto the
already-seeded node the next startup (`EnsureSeededInstagram`, only while the field is
blank). Meta's business discovery hands over the recent
...
## Visitor accounts, reviews and favourites

A visitor signs in with Google or with a one-time link sent to their email, and can then
rate and review an establishment (`place` or `mall`, `RATEABLE_TYPES` / `ReviewRules.PlaceTypes`)
and keep it in "Mis favoritos" (`/{ciudad}/favoritos`). Wherever a place carries both, the
portal's rating is shown **beside** Google's and never averaged with it — two crowds, two
badges, each named (`RatingPair` in `components/RatingBadge.tsx`): the detail header, the
"Opiniones" section, every listing card (`Rating`, now an async Server Component reading
one cached map of every rated place), the listing map popups and rows (`markerOf`) and
"¿Qué está cerca?" (`/api/nearby` returns `siteRating` beside `rating`). The JSON-LD
`aggregateRating` still states Google's.

**Who owns what.** The portal owns the session: a signed stateless cookie (`lib/session.ts`,
`jose`, `AUTH_SECRET`) carrying the member key and the display name, and the Google OAuth
round trip with PKCE (`arctic`, `app/api/auth/google/*`, redirect URI
`{NEXT_PUBLIC_SITE_URL}/api/auth/google/callback`). The CMS owns the people and the data:
every visitor is an Umbraco **member** keyed by the email address both methods prove
(`AccountController` finds or creates it), reviews and favourites live in two tables of
their own (`cityGuideReview`, `cityGuideFavorite`, created by the `UserContentMigration`
plan on startup — not content nodes: public UGC would drag versions, the content cache and
the Delivery API index along), and `ReviewStore` holds the per-place summaries in memory
until a write. The email link is issued and redeemed in the CMS because that is where the
SMTP is: a random token whose hash sits in the memory cache for 30 minutes, single use,
redeemed by a **button** on `/{ciudad}/acceder` rather than by the visit, since mail
scanners open links. The name typed when asking for it becomes the member's name; Google's
is used only when the member is created.

**The trust boundary** is `[PortalApi]` (`PortalApiAttribute`): `/api/account/*` and
`/api/reviews/*` answer only a request carrying `x-portal-secret` equal to
`CityGuide:PortalApiSecret` (the frontend's `CMS_PORTAL_SECRET`), and 404 to anyone else or
when unset. The portal's server — Server Actions and route handlers, never the browser —
names the member it acts for; the CMS re-checks on every write that the member is approved
and not locked out and that the place is a published establishment. A stateless cookie
cannot be revoked, and that re-check is what revokes it.

**Caching.** Pages stay public and cached: the reviews list and every rating are fetched
with the `reviews` tag, and what is personal (am I signed in, my review, my favourite) is
asked from the browser (`/api/me`, `/api/me/place/[id]`) — reading the cookie in the city
layout would have made every page of the portal dynamic. A write drops the tag at once from
its Server Action (`updateTag`); a moderation change reaches the frontend through
`FrontendRevalidator` (`/api/revalidate?tag=reviews`), the same class the content
invalidator now uses.

**Moderation.** Reviews publish at once. The "Reseñas" dashboard in the Content section
(`wwwroot/App_Plugins/CityGuideReviews`, a plain ES module on the backoffice import map,
over `ReviewModerationController` in the Management API) lists the newest reviews and hides
or restores one. Blocking a person is Umbraco's own: clear "Approved" or lock the member in
the Members section, and `MemberReviewSync` copies that standing (and the name) onto their
reviews, which go off the portal, while sign-in and writes are refused. Deleting a member
deletes their reviews and favourites.

**Configuration**, all runtime and none of it at build time: CMS `CityGuide__PortalApiSecret`
(plus the existing `FrontendBaseUrl`/`RevalidateSecret` and SMTP); web app `AUTH_SECRET`,
`CMS_PORTAL_SECRET`, and `GOOGLE_CLIENT_ID`/`GOOGLE_CLIENT_SECRET` for the Google button
(without them only the email link is offered). Without the two secrets nobody can sign in
and the account controls do not render.
...
- Deliberate v1 omissions (do not build unasked): agent photo upload, webhook-driven revalidation.
